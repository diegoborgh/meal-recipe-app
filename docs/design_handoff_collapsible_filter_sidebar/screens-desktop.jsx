...
// ─────────────────────────────────────────────────────────────
// Desktop · Search results — collapsible filter sidebar + grid
// Click the chevron in the sidebar header (expanded) or rail (collapsed)
// to toggle. Collapsed state shows a 64px rail with an active-count badge
// + the active filters as removable chips above the results grid.
// ─────────────────────────────────────────────────────────────
function D_Results() {
  const t = T();
  ...
  // Active filters — derived from the mock selection. In production this
  // would come from the search state object.
  const activeFilters = [
    { group: 'Diet',  label: 'Vegetarian' },
    { group: 'Avoid', label: 'Dairy' },
    { group: 'Time',  label: 'Under 30 min' },
    { group: 'Meal',  label: 'Dinner' },
    { group: 'Cal',   label: '200–600 kcal' },
  ];
  ...
  return (
    <DesktopShell active="home">
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
        {expanded ? (
          <FilterSidebarExpanded onCollapse={() => setExpanded(false)} />
        ) : (
        ...
        {/* Results */}
        <div style={{ flex: 1, padding: '32px 40px', overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {/* When collapsed, surface the active filters here so users still
              see + remove them without re-expanding the sidebar. */}
          {!expanded && activeFilters.length > 0 && (
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', color: t.inkSoft, marginRight: 4 }}>
                Filters
              </span>
              {activeFilters.map((f) => (
                <Chip key={f.label} active onRemove={() => {}}>{f.label}</Chip>
              ))}
              <button onClick={() => setExpanded(true)} style={{
                background: 'none', border: 'none', color: t.accent, fontSize: 12,
                fontWeight: 600, cursor: 'pointer', padding: '0 8px', height: 32,
              }}>Edit all</button>
            </div>
          )}
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: expanded ? 'repeat(3, 1fr)' : 'repeat(4, 1fr)',
            gap: 18,
          }}>
            {recipes.slice(0, expanded ? 6 : 8).map((r) => <RecipeCard key={r.id} {...r} />)}
          </div>
        </div>
      </div>
    </DesktopShell>
...
// Expanded filter sidebar (280px)
const FilterSidebarExpanded = ({ onCollapse }) => {
  const t = T();
  return (
    <div style={{
      width: 280, padding: '28px 28px', borderRight: `1px solid ${t.line}`,
      overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 22, flexShrink: 0,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <button
            ...
            title="Collapse"
            style={{
              width: 28, height: 28, marginLeft: 4, borderRadius: 8,
              background: 'transparent', border: 'none', cursor: 'pointer',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: t.inkSoft,
            }}>
            <Icon name="arrowL" size={16} strokeWidth={2} />
          </button>
        </div>
      </div>
...
// Collapsed rail — 64px, icon + active-count badge
const FilterSidebarCollapsed = ({ count, onExpand }) => {
  const t = T();
  return (
    <div style={{
      width: 64, padding: '28px 0', borderRight: `1px solid ${t.line}`,
